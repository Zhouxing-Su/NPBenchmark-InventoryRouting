...
struct TspCacheBase {
    using ID = Graph::ID;
    using Weight = double;
    using NodeList = Graph::List<ID>; // the `nodes` should be ordered in increasing order during the traverse.
    using NodeSet = Graph::List<bool>; // the size of the set should be the same as the node number.
